...
func (s *SSHServer) handleTunnelCommand(sess ssh.Session) {
	key := s.genKey()
	s.logger.Debug("creating tunnel", "key", key)
	...
	if err != nil {
		s.logger.Error("failed to write to ssh session", "error", err)
		return
	}
	...
	s.logger.Debug("transfer over tunnel complete", "key", key)
...
func (s *SSHServer) handleSessionWithCommand(sess ssh.Session) {
	...
	case CmdTunnel:
		...
		if err != nil {
			s.logger.Error("failed to parse tunnel command", "error", err)
			return
		}
		if !shouldTunnel {
	...
	case CmdTTL:
		...
		if err != nil {
			s.logger.Error("failed to parse ttl command", "error", err)
			return
		}
...
func (s *SSHServer) handleTTLCommand(sess ssh.Session, userTTL int) {
	...
	if ttl < MinTTL {
		s.logger.Debug("user ttl too low; falling back on MinTTL", "ttl", ttl, "min_ttl", MinTTL)
		ttl = MinTTL
	}
	if ttl > MaxTTL {
		s.logger.Debug("user ttl too high; falling back on MaxTTL", "ttl", ttl, "max_ttl", MaxTTL)
		ttl = MaxTTL
	}
...
func (s *SSHServer) handleBasicSession(sess ssh.Session, ttl time.Duration) {
	...
	if err != nil && err != io.EOF && err != io.ErrUnexpectedEOF {
		s.logger.Error("failed to read from ssh session", "error", err)
		return
	}

	key := s.genKey()
	s.logger.Debug("writing to store", "key", key, "code", string(buf[:n]))
	err = s.store.Set(sess.Context(), key, string(buf[:n]), ttl)
	s.logger.Debug("wrote bytes", "bytes", n)
	if err != nil {
		s.logger.Error("failed to write to redis",
			"error",
			err,
			"key",
			key,
	...
	if err != nil {
		s.logger.Error("failed to increment code uploaded count", "error", err)
		return
	}
...
func (s *SSHServer) HandleSession(sess ssh.Session) {
	...
	if rateLimited {
		s.logger.Info("rate limited", "ip", sess.RemoteAddr().String())
